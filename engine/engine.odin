...
APPLICATION_NAME :: "Odin Vulkan OpenXR"
ENGINE_NAME :: "No Engine"
DEFAULT_WIDTH :: 1920
DEFAULT_HEIGHT :: 1080

// Engine Vulkan Resources
Vk_Subsystem :: struct {
...
engine_create_instance :: proc(window: ^SDL.Window) -> (instance: vk.Instance) {
...
engine_create_debug_messenger :: proc(instance: vk.Instance) -> (debug_messenger: vk.DebugUtilsMessengerEXT) {
...
engine_create_surface :: proc(window: ^SDL.Window, instance: vk.Instance) -> (surface: vk.SurfaceKHR) {
...
engine_check_physical_device :: proc(
	physical_device: vk.PhysicalDevice,
	surface: vk.SurfaceKHR,
) -> (
	queue_index: u32,
...
engine_pick_physical_device :: proc(
	instance: vk.Instance,
	surface: vk.SurfaceKHR,
) -> (
	physical_device: vk.PhysicalDevice,
	...
	for dev in devices {
		dev_queue, ok := engine_check_physical_device(dev, surface)
		if !ok {continue}
...
engine_create_image_views :: proc(
	device: vk.Device,
	images: []vk.Image,
	format: vk.Format,
) -> (
...
engine_create_shader_module :: proc(device: vk.Device, src: []u8) -> (module: vk.ShaderModule) {
...
engine_create_render_pass :: proc(device: vk.Device, format: vk.Format) -> (render_pass: vk.RenderPass) {
...
engine_create_graphics_pipeline :: proc(
	device: vk.Device,
	render_pass: vk.RenderPass,
) -> (
	pipeline: vk.Pipeline,
	...
	vert_module := engine_create_shader_module(device, vertex_main_src[:])
	frag_module := engine_create_shader_module(device, pixel_main_src[:])
...
engine_create_framebuffers :: proc(
	device: vk.Device,
	render_pass: vk.RenderPass,
	extent: vk.Extent2D,
	image_views: []vk.ImageView,
...
engine_create_command_pool :: proc(device: vk.Device, queue_index: u32) -> (command_pool: vk.CommandPool) {
...
engine_create_command_buffers :: proc(
	device: vk.Device,
	command_pool: vk.CommandPool,
) -> (
	command_buffers: [2]vk.CommandBuffer,
...
engine_record_command_buffer :: proc(
	device: vk.Device,
	framebuffers: []vk.Framebuffer,
	pipeline: vk.Pipeline,
	render_pass: vk.RenderPass,
...
engine_create_sync_objects :: proc(
	device: vk.Device,
) -> (
	image_available_semaphores: [2]vk.Semaphore,
	render_finished_semaphores: [2]vk.Semaphore,
...
engine_init :: proc() -> (engine: Engine) {
	...
	instance := engine_create_instance(window)
	debug_messenger := engine_create_debug_messenger(instance)
	surface := engine_create_surface(window, instance)
	physical_device, queue_index := engine_pick_physical_device(instance, surface)
	device, graphics_queue := engine_create_device(instance, physical_device, queue_index)
	swapchain, images, format, extent := engine_create_swapchain(physical_device, device, surface)
	image_views := engine_create_image_views(device, images, format)
	render_pass := engine_create_render_pass(device, format)
	pipeline, pipeline_layout := engine_create_graphics_pipeline(device, render_pass)
	framebuffers := engine_create_framebuffers(device, render_pass, extent, image_views)
	command_pool := engine_create_command_pool(device, queue_index)
	command_buffers := engine_create_command_buffers(device, command_pool)
	image_available_semaphores, render_finished_semaphores, in_flight_fences := engine_create_sync_objects(device)
...
engine_quit :: proc(engine: ^Engine) {
	using engine
	xr_subsystem_quit(&xr_subsystem)
...
engine_event :: proc(engine: ^Engine, event: SDL.Event) {
